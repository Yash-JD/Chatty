...
export const useChatStore = create((set, get) => ({
  ...
  respondToFriendRequest: async (requestId, action) => {
    try {
      const res = await axiosInstance.put(`/friends/respond/${requestId}`, {
        action,
      });
  ...
  subscribeToFriendRequests: () => {
    ...
    socket.on('friendRequestAccepted', (newFriend) => {
      ...
      set({
        sentRequests: get().sentRequests.filter(
          (req) => req.receiver?._id !== newFriend._id,
        ),
      });
  ...
  setShowFriendPanel: (showFriendPanel) =>
    set({ showFriendPanel, selectedUser: null }),
  setSelectedUser: (selectedUser) =>
    set({ selectedUser, showFriendPanel: false }),
}));
